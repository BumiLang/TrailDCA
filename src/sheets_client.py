...
PERCENT_COLUMNS = ["수익률", "최고수익률", "익절기준", "직전매수수익률"]
...
class SheetsClient:
    ...
    def _default_row_values(
        ...
        last_buy_rate_pct: str | None = None,
        sell_stage: str = "0",
    ) -> list[str]:
        ...
        history yet). 직전매수수익률 defaults to 최고수익률 when not given
        explicitly -- no buy has happened yet through this bot for a
        brand-new symbol, so there's nothing better to seed the ratchet
        from."""
        if last_buy_rate_pct is None:
            last_buy_rate_pct = peak_rate_pct
