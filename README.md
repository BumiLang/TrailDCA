...
- **익절기준(트레일링 스탑)**: 최고수익률(peak)이 10%(`PEAK_ACTIVATION_RATE`)에 도달하면 활성화되어 정확히 5%에서 시작하고, peak 30%(`TAKE_PROFIT_BREAKPOINT`)까지는 선형으로 증가해 21%에 도달, 그 이후로는 `peak × 0.7`(`TAKE_PROFIT_HIGH_SLOPE`)로 계산됩니다. 최고수익률은 절대 내려가지 않고, 활성화 전에는 익절기준이 갱신되지 않습니다.
  - **누적 매입금액이 10만원(`DAILY_BUY_TARGET_KRW`) 미만인 동안**: 최고수익률은 평소처럼 매 틱 계속 갱신되지만, 익절기준은 -100%(`INITIAL_TAKE_PROFIT_THRESHOLD`)로 고정되어 활성화되지 않습니다 — 어차피 이 구간에서는 청산 자체가 불가능하므로(아래 참고), peak가 미리 10%를 넘겨 활성화되어 있으면 정작 청산이 가능해지는 순간 현재 수익률보다 높은 익절기준이 이미 대기 중이어서 곧바로 청산되는 문제를 막기 위함입니다.
  - **누적 매입금액이 10만원을 처음 넘기는 틱**: 그 시점까지 쌓인 최고수익률을 버리고, 그 순간의 현재 수익률로 최고수익률을 다시 시작합니다. 이후부터는 이 새 최고수익률을 기준으로 익절기준이 정상적으로 갱신됩니다.
...
- **수동 재매수 감지**: `청산여부=TRUE`인데 실계좌 조회 결과 다시 그 종목을 보유하고 있으면, `청산여부`를 `FALSE`로 되돌리고 보유수량/매입금액 등을 실제 값으로 갱신하며, **최고수익률/익절기준을 현재 수익률 기준으로 새로 시드**합니다(청산 직전의 낡은 값을 이어받지 않음). 이 재시딩도 위 "누적 매입금액 10만원" 게이팅을 그대로 따릅니다 — 재진입 매입금액이 10만원 미만이면 최고수익률만 현재 수익률로 시작하고 익절기준은 -100%로 고정, 재진입 한 방에 10만원을 넘겼다면 그 자리에서 바로 정상 공식으로 활성화됩니다. 또한 그날 봇 자신의 DCA 매수는 건너뜁니다 — 사람이 방금 수기로 재진입했으므로 같은 거래일에 봇이 5,000원을 또 얹어 사지 않도록 "오늘 매수 완료"로 표시해둡니다.

⚠️ **참고**: 시트에서 `익절기준`(또는 `최고수익률` 등) 셀을 사람이 직접 수정해도 대부분 효과가 없습니다. 시트는 하루 한 번(08:00 KST) 또는 서비스 재시작 시에만 다시 읽히고, 그마저도 최고수익률이 이미 10%를 넘겨 활성화된 종목은 매 틱 `peak` 값만으로 익절기준을 다시 계산해 덮어씁니다. 매입금액이 10만원 미만인 종목은 사람이 `익절기준`에 어떤 값을 넣어놔도 다음 틱에 무조건 -100%로 다시 덮어써집니다(위 "누적 매입금액이 10만원 미만인 동안" 규칙). 익절기준 산식 자체를 바꾸려면 `src/config.py`의 `TAKE_PROFIT_*` 상수를 수정하세요.
