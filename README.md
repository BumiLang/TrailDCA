...
- **청산**: 최고수익률이 10% 이상이고 현재 수익률이 익절기준 이하로 떨어지면 전량 매도.
- **소수점 매수 미지원 종목**: 5,000원 금액 주문이 실패하면 1주 통매수로 폴백하되, "1주를 추가로 샀을 때 예상되는 수익률"이 `max(10%, 현재 익절기준)` 이상일 때만 진입합니다 (그 미만이면 이번 시도는 건너뛰고 다음 재시도 주기에 다시 판단). 이 전략이 관리하는 종목은 이미 포지션이 있다는 전제이며, 최초 진입(첫 매수)은 수동으로 하는 것을 가정합니다.
